...
function Dashboard() {
  ...
  if (token){
    ...
    var user_id = decoded_token.user_id
    var username = decoded_token.username
    var full_name = decoded_token.full_name
    var image = decoded_token.image
    var bio = decoded_token.bio
  }
  ...
  useEffect(() => {
    const fetchPostData = async () => {
      try {
        const response = await api.post("dashboard/", {"text": "Hello"})
        setRes(response.data.response)
      }
      catch (error){
        console.log(error);
